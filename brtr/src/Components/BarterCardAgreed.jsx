...
function BarterCardAgreed({barter}) {
  ...
  <div>
    <div className= "mx-auto mt-5 p-10 h-full w-4/6 hover:bg-slate-200 bg-opacity-70 rounded-xl transition ease-in-out duration-200 bg-slate-100">
     ...
     <p className="text-2xl">You and &nbsp;</p>
     <p className="text-2xl font-extrabold text-indigo-700 "> {barter?.recipient_hours} hours&nbsp;</p> 
     <p className="text-2xl ">of {barter?.recipient_skill?.name} from {barter?.recipient?.first_name}</p> 
     </div>
     ...
     <p className="text-2xl mb-5 tex">of {barter?.proposer_skill?.name}</p> 
     </div>
     ...
     <button className="inline-block px-6 py-3 leading-none text-white rounded shadow bg-amber-500 hover:bg-indigo-600 hover:-translate-y-1">View {barter?.recipient?.first_name} Profile</button>

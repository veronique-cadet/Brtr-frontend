...
import BarterCardAgreed from './BarterCardAgreed';

function UserProfileTwo({user, setUser, yourBarters, setYourBarters}) {
...
const [barterClicked, setBarterClicked] = useState(true)
  ...
  const barter = useLocation();
  const { from } = barter.state?.from;
  const id = barter.state?.from.id;
  console.log(id)
  ...
  <div className="md:flex no-wrap md:-mx-2 ">
    ...
    <div className="w-full md:w-3/12 md:mx-2">
      ...
      <div className="p-3 bg-white border-t-4 border-indigo-400  rounded-3xl  hover:shadow-xl">
        ...
        <h1 className="my-1 text-xl font-bold leading-8 text-gray-900">{userProfile?.proposer?.first_name}</h1>
        <h3 className="leading-6 text-gray-600 font-lg text-semibold">{userProfile?.proposer_skill?.name}</h3>
        <p className="text-sm leading-6 text-gray-500 hover:text-gray-600">{userProfile?.proposer?.bio}</p>
        <ul className="px-3 py-2 mt-3 text-gray-600 bg-gray-100 divide-y rounded shadow-sm hover:text-gray-700 hover:shadow"><li className="flex items-center py-3">
          ...
          <span className="ml-auto"><span className="px-2 py-1 text-sm text-white bg-indigo-500 rounded">{userProfile?.proposer?.rating}/5</span></span>
          </li>
        ...
        <div className="text-gray-700">
          <div className="grid text-sm md:grid-cols-2">
            <div className="grid grid-cols-2">
              ...
              <div className="px-4 py-2">{userProfile?.proposer?.first_name}</div>
            </div>
            <div className="grid grid-cols-2">
            ...
            <div className="grid grid-cols-2">
              ...
              <div className="px-4 py-2">
                <a className="text-blue-800" href="mailto:jane@example.com">{userProfile?.proposer?.first_name}</a>
              </div>
            </div>
            <div className="grid grid-cols-2">
    ...
    <div className="w-full md:w-3/12 md:mx-2">
      ...
      <div className="p-3 bg-white border-t-4 border-indigo-400">
        <div className="overflow-hidden image">
          <img className="w-full h-auto mx-auto" src="https://cdn.britannica.com/10/172210-050-24612D72/Cyndi-Lauper-Tony-Award.jpg?w=400&amp;h=300&amp;c=crop" alt=""/></div>
        <h1 className="my-1 text-xl font-bold leading-8 text-gray-900">{userProfile?.proposer?.first_name}</h1>
        <h3 className="leading-6 text-gray-600 font-lg text-semibold">{userProfile?.proposer?.first_name}</h3>
        <p className="text-sm leading-6 text-gray-500 hover:text-gray-600">{userProfile?.proposer?.first_name}</p>
        <ul className="px-3 py-2 mt-3 text-gray-600 bg-gray-100 divide-y rounded shadow-xl hover:text-gray-700 hover:shadow-xl"><li className="flex items-center py-3">
          <span contenteditable="false">Rating</span>
          <span className="ml-auto"><span className="px-2 py-1 text-sm text-white bg-indigo-500 rounded">{userProfile?.proposer?.first_name}/5</span></span>
          </li>
          <li className="flex items-center py-3">
            <span contenteditable="false">Bartrs</span>
            <span className="ml-auto">10 Bartrs</span>
...
<select className="px-4 py-3.5 w-96 text-gray-400 font-medium placeholder-gray-400 bg-white outline-none border border-gray-300 rounded-lg focus:ring focus:ring-indigo-300">
  <option value="volvo">{userProfile?.proposer?.first_name}</option>
</select>
</div>

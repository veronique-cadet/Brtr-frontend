...
import CalendarAgreedCard from './CalendarAgreedCard'

function CalendarAgreed({setUser, user, yourBarters, setYourBarters,}) {
  ...
  const filteredBartersCalender = yourBarters.filter((barter) => {
  ...
  console.log(filteredBartersCalender)
  
const barterCardCalendar = filteredBartersCalender.map((barter) => {
return <CalendarAgreedCard key= {barter.id} barter={barter} user={user}/> })
 ...
 <div className="container px-4 mx-auto">
   ...
   <Link to="/calendar">
     ...
     <button className=" ml-7 pt-5  pb-10 group relative flex-col items-center text-lg  text-amber-500 hover:text-transparent font-bold bg-clip-text hover:bg-gradient-to-r from-amber-500">Agreed Bartrs</button></Link>
